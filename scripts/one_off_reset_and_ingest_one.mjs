...
function makeLongKoreanGuide() {
  return [
    ...
    '',
    '### FAQ',
    '1) 지금 사도 되나요, 기다릴까요? → “당장 필요”면 구매, 여유가 있으면 한국 가격/프로모션 확인 후 결정이 안전합니다.',
    '2) 이전 모델에서 업그레이드 가치가 있나요? → CPU보다 RAM/SSD/배터리 상태가 체감에 더 큰 경우가 많습니다.',
    '3) 직구는 어떤 경우에 유리한가요? → 총비용과 보증/AS 리스크를 비교해 결정하세요.',
    '4) 어떤 옵션 구성이 무난한가요? → 내 병목이 메모리/저장공간인지 먼저 판단하세요.',
    '5) 이번 업데이트에서 가장 중요한 포인트는? → 내 사용환경에서 “체감”이 생기는 변화인지입니다.',
    '',
    '### 댓글 질문',
...
async function buildPost(item, sourceName, category) {
  ...
  // Hard padding to guarantee >= 3000 chars (avoid one-line walls; keep readable bullets)
  while (bodyLen < 3000) {
    const extra = [
      '### 체크포인트 요약(짧게 다시 보기)',
      '- “지금 불편함”이 명확하면: 구매 우선',
    ...
    sections.splice(sections.length - 1, 0, { heading: '추가 요약', content: extra })
