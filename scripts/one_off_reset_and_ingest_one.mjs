...
function makeLongKoreanGuide() {
  ...
  return [
    ...
    '  - 실전: 하루 사용 동선을 적어보면(학교/카페/회사/이동) “배터리+무게”가 얼마나 중요한지 바로 보입니다.',
    '',
    '- 시나리오 B: 개발/코딩(빌드, 도커, 멀티태스킹, 외부 모니터)',
    '  - 중요: 메모리(RAM), SSD 속도/용량, 장시간 부하에서의 열관리, 포트/허브 환경',
    '  - 팁: CPU보다 RAM/SSD가 병목인 경우가 많습니다. 옵션 구성이 체감에 더 큽니다.',
    '  - 실전: 브라우저 탭+IDE+도커+메신저를 동시에 켜는 습관이 있으면, RAM이 부족할 때 체감이 급격히 나빠집니다.',
    '',
    '- 시나리오 C: 크리에이터(사진/영상/디자인)',
    '  - 중요: 미리보기/렌더링 시간, 대용량 파일 I/O, 색 정확도, 외부 스토리지/모니터',
    '  - 팁: 작업 해상도와 도구에 따라 상위 라인이 필요할 수 있습니다.',
    '  - 실전: 4K 영상/RAW 사진을 자주 다루면 저장장치 속도와 용량이 체감을 좌우합니다.',
    '',
    ...
    '',
    '### 비교 관점(이전 모델/대체재)',
    '성능이 좋아졌다는 말만 보고 결정하면 실수하기 쉽습니다. 비교할 때는 같은 가격대의 대체재와 이전 모델을 함께 놓고 보는 게 좋습니다.',
    '비교 항목은 (1) 체감 성능(내 작업 기준), (2) 배터리/휴대성, (3) 화면/스피커/웹캠, (4) 포트/확장성, (5) AS/보증, (6) 리셀가입니다.',
    '이 중 2~3개만 확실히 이기는 포인트가 있다면 업그레이드가 합리적일 확률이 높습니다.',
    '',
    '### 체크리스트 한 장 요약(저장용)',
    '- 한국 정발/가격/출시일이 확정되었나?',
    '- 내 병목(메모리/저장공간/배터리/발열)이 무엇인가?',
    '- 이번 모델이 그 병목을 해결하는가?',
    '- AppleCare+ 포함 총비용은 얼마인가?',
    '- 직구 vs 정발의 보증 차이는 감당 가능한가?',
    '- 지금 사야 하는 이유가 충분한가? (Need/W ant 구분)',
    '',
    '### 댓글 질문',
